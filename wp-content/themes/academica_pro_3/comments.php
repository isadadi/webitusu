...
<?php if ( have_comments() ) : ?>
	...
	<ol class="commentlist">
		<?php
			 ...
			 */
			wp_list_comments( array( 'callback' => 'academica_comment' ) );
		?>
	</ol>
...
$custom_comment_form = array( 'fields' => apply_filters( 'comment_form_default_fields', array(
    'author' => '<div class="form_fields clearfix"><p class="comment-form-author">' .
			...
			esc_attr( $commenter['comment_author'] ) . '" size="30"' . $aria_req . ' class="required" placeholder="' . __( 'Name', 'wpzoom' ) . ( $req ? ' *' : '' ) . '" />' .

 			'</p>',
    'email'  => '<p class="comment-form-email">' .
			//'<label for="email">' . __( 'Email Address' , 'wpzoom' ) . '</label> ' .
            //( $req ? '<span class="required_lab">*</span>' : '' ) .
 			'<input id="email" name="email" type="text" value="' . esc_attr(  $commenter['comment_author_email'] ) . '" size="30"' . $aria_req . ' class="required email" placeholder="' . __( 'Email Address', 'wpzoom' ) . ( $req ? ' *' : '' ) . '" />' .
 			'</p>',
    'url'    =>  '<p class="comment-form-url">' .
			//'<label for="url">' . __( 'Website' , 'wpzoom' ) . '</label> ' .
 			'<input id="url" name="url" type="text" value="' . esc_attr(  $commenter['comment_author_url'] ) . '" size="30"' . $aria_req . ' placeholder="' . __( 'Website', 'wpzoom' ) . '" />' .
